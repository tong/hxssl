...
static  value hxssl___SSL_read( value ssl ) {
	printf("hxssl___SSL_read\n");
	//TODO buffer size
	buffer b;
	const int bufsize = 256; //TODO
	char buf[bufsize];
	int len;
	b = alloc_buffer(NULL);
	while( true ) {
		len = SSL_read( val_ssl(ssl), buf, 256 );
		//if( len == SOCKET_ERROR )
		//	return block_error();
		if( len == 0 )
			break;
...
static value hxssl___SSL_write( value ssl, value data ) {
	...
	printf("hxssl___SSL_write\n");
	
	//const char *cdata;
	//int datalen,
	int slen;

	const char *cdata = val_string( data );
	int datalen = val_strlen( data );
	...
	printf("wwwwwwwwwwwwwwwwwwwww %s\n", cdata );
	while( datalen > 0 ) {
		//POSIX_LABEL(write_again);
		//slen = send(val_sock(o),cdata,datalen,MSG_NOSIGNAL);
		
		printf(">>>>>>>>>>>>> %s %i >>>\n", cdata, datalen );
		slen = SSL_write( val_ssl(ssl), cdata, datalen );
		//printf("LEEEN %i\n", slen );
		/*
...
static value hxssl___SSL_accept( value ssl ) {
	//TODO
	printf("Socket.accept 111\n");
	SSL* _ssl = val_ssl( ssl );
	//SSL_set_accept_state( _ssl );
	int client;
	SSL_set_fd( _ssl, client );
	printf("Socket.accept 222 %i\n",client);
	SSL_accept( _ssl );      
	printf("Socket.accept 333\n");
	return VAL_NULL;
}
